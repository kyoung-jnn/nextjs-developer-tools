 ...
 * 탭 상태 생성 또는 조회
 */
export async function getOrCreateTabState(tabId: number): Promise<TabState> {
 ...
 * 페이지 상태 조회
 */
export async function getPageState(tabId: number): Promise<PageState | null> {
 ...
 * DevTools 연결 상태 업데이트
 */
export async function setDevToolsConnected(
  tabId: number,
  connected: boolean
): Promise<void> {
 ...
 * 닫힌 탭의 상태 정리
 */
export async function cleanupClosedTabs(): Promise<number> {
