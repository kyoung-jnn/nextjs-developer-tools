 ...
 */
export function extractPayload(): ExtractionResult {
 ...
 */
export function extractPageRouterPayload(): ExtractionResult {
 ...
 */
export function extractAppRouterPayload(): ExtractionResult {
  const chunks: string[] = [];
 ...
 */
export async function extractPayloadWithRetry(
  maxAttempts = 3,
  delayMs = 500
 ...
 */
export function parseRSCChunks(rawData: string): Array<{
  index: number;
  type: string;
  data: unknown;
}> {
 ...
 */
function determineChunkType(data: unknown): string {
  if (typeof data === "string") {
    if (data.startsWith("$")) {
      return "reference";
    }
